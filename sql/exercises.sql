...
SELECT * FROM category WHERE category_id > 4 AND name REGEXP "^(C|S|T)";
...
# for the first 10 films with behind the scenes footage under 2 hours in length and a rental duration between 5 and 7 days, ordered by length in descending order.
SELECT 
    title,
    description,
    special_features,
    ...
    rental_duration
FROM film
WHERE FIND_IN_SET("Behind the Scenes", special_features) AND length < 120 AND rental_duration BETWEEN 5 AND 7 ORDER BY length DESC LIMIT 10;
...
# 9a. Select the customer first_name/last_name and actor first_name/last_name columns from performing a left join between the customer and actor column on the last_name column in each table. (i.e. `customer.last_name = actor.last_name`)
# Label customer first_name/last_name columns as customer_first_name/customer_last_name
# Label actor first_name/last_name columns in a similar fashion.
SELECT 
    actor.first_name actor_first_name,
    actor.last_name actor_last_name,
    customer.first_name customer_first_name,
    customer.last_name customer_last_name
FROM customer LEFT JOIN actor USING (last_name);
...
SELECT
    ...
    customer.first_name customer_first_name,
    customer.last_name customer_last_name
FROM customer RIGHT JOIN actor ON actor.last_name = customer.last_name;
...
SELECT
    ...
    customer.first_name customer_first_name,
    customer.last_name customer_last_name
FROM customer INNER JOIN actor ON actor.last_name = customer.last_name;
...
SELECT
    city,
    country
FROM city LEFT JOIN country USING (country_id);
